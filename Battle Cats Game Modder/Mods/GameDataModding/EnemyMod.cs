...
namespace Battle_Cats_Game_Modder.Mods.GameDataModding
{
    public class EnemyMod
    {
        public static void EnemyCSV()
        {
            ...
            List<List<int>> enemy_data = ReadIntCSV(path);
            string[] values =
            {
            ...
            for (int k = 0; k < enemyIDs.Length; k++)
            {
                ...
                ColouredText($"&What do you want to edit in enemy id: &{Enemyid}&? (You can enter multiple ids separated by spaces to edit multiple at once)\n");
                string[] answer = Console.ReadLine().Split(' ');
                for (int i = 0; i < answer.Length; i++)
                {
                    int traitID = int.Parse(answer[i]);
                    ColouredText($"&What value do you want to set &{values[traitID]}& to? (for flags, 1 = on, 0 = off):");
                    int value = Inputed();
                    enemy_data[Enemyid][traitID] = value;
                }
                WriteCSV(enemy_data, path, true);
            }
            ColouredText($"&Finished, wrote data to: &{path}&\n");
        }
    }
}
