...
namespace Battle_Cats_Game_Modder.Mods.GameDataModding
{
    public class UnitMod
    {
        public static void Unitcsv()
        {
            ...
            ColouredText($"What do you want to edit?:\n{CreateOptionsList<string>(forms)}{multipleVals}\n");
            ...
            foreach (string selectedForm in answer)
            {
                ...
                ColouredText($"{CreateOptionsList(values, unitData[choice - 1].ToArray())}What do you want to edit for the {forms[choice - 1]}?:{multipleVals}\n");
                answer = Console.ReadLine().Split(' ');
                foreach (string idS in answer)
                {
                    int id = int.Parse(idS);
                    ColouredText($"&What do you want to set &{values[id - 1]}& to?:(for flags, 1 = enable, duration/time = frames, chance = 0->100)\n");
                    int val = (int)Inputed();
                    unitData[choice - 1][id - 1] = val;
                }
            }
            WriteCSV(unitData, path, true);
        }
